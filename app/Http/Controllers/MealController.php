    ...
    function listMeals(Request $request, $country){
        ...
        if($request->query->get('calories')){
            ...
            if($request->query->get('category')){
                return response()->json(Meal::where("countrycode",$country)->where("category",$request->query->get('category'))->whereBetween("total_calories",[$request->query->get("min-calories")?(double)$request->query->get("min-calories"):0,$request->query->get("max-calories")?(double)$request->query->get("max-calories"):10000000000])->get($data), 200, $this->headers);
            }
        ...
        if($request->query->get('category')){
            return response()->json(Meal::where("countrycode",$country)->where("category",$request->query->get('category'))->get($data), 200, $this->headers);
        }
